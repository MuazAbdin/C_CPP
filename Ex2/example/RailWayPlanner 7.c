	...
	while (fgets(input, sizeof(input), file) != NULL)
	{
		...
		else
		{
			...
			if (numParts == capacity)
			{
				...
				railInfo.partsCollection = (railPart *)realloc(railInfo.partsCollection, capacity * sizeof(railPart));
			}

			sscanf(partDetails[0], "%c", &railInfo.partsCollection[numParts].start);
			sscanf(partDetails[1], "%c", &railInfo.partsCollection[numParts].end);

			char *lenPtr, *pricePtr;
			railInfo.partsCollection[numParts].length = (int) strtol(partDetails[2], &lenPtr, 10);
			railInfo.partsCollection[numParts].price = (int) strtol(partDetails[3], &pricePtr, 10);

			printf("[%c][%c][%d][%d]\n", railInfo.partsCollection[numParts].start, railInfo.partsCollection[numParts].end,
				   railInfo.partsCollection[numParts].length, railInfo.partsCollection[numParts].price);
		}
		line++;
	}
	return 1;
...
int main(int argc, char *argv[])
{
	...
	checkDetails(inputFile, inputDetails);
	fclose(inputFile);
	return 0;
}