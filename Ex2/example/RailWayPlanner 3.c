...
char **checkPartDetails(char part[], char **details)
{
//	char *partDetails[NUM_PART_DETAILS];
	printf("\n(( %p )) --[%p  %p %p %p] \n", partDetails, partDetails[0], partDetails[1], partDetails[2],
			partDetails[3]);

	char *ptr = strtok(part, ",");
	int idx = 0;
	while (ptr != NULL)
	{
		partDetails[idx] = (char *)malloc(strlen(ptr) * sizeof(char) + 1);
		printf("\n(( %p )) --[%p  %p %p %p] \n", partDetails, partDetails[0], partDetails[1], partDetails[2],
			   partDetails[3]);

		printf("size of (%s) = %lu\n", ptr, strlen(ptr));
//		printf("size of = %lu\n", sizeof(partDetails));
		strncpy(partDetails[idx], ptr, strlen(ptr) + 1);
...
//			}
			...
			*partDetails = checkPartDetails(input);
			printf("LINE (%d) -> ", line);
			printf("%p", *partDetails);
//			printf("%s\n", *partDetails);
