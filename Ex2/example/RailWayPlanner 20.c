...
//	printf("%s   /////// \n", arr);
	char *cutInput = (char*)malloc((int) strlen(arr) * sizeof(char));
...
//			printf("[%s] ", joints);
//			printf("%c\t", detail[0]);
//			printf("%lu\t", strlen(&detail[0]));
//			printf("joint not in array, len>1 \n");
			return false;
		}
	}
...
//	int collectionSize = capacity;
	...
	while (fgets(input, sizeof(input), file) != NULL)
	{
		...
		{
//			printf("ATTT\n");
//			printf("{ %s }\n{ %s }\n{ %s }\n", input, *partDetails, railInfo -> kindsJoints);
...
//	// Total number of parts is total number of lines minus first three and last empty line.
