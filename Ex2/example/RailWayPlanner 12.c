...
{
	int minCost = INF;
	railPart *optimalJoint = (railPart *)calloc(1, sizeof(railPart));
	...
	for (int r = 0; r < rows; r++)
	{
		for (int c = 0; c < columns; c++)
		{
			...
			else if (table[(r - 1) * columns + c] == 0 || table[(r - 1) * columns + c] == INF)
			{
				...
				if (currentPart->length == 0)	// all struct fields are 0
				{
...
//}
 ...
 int minimumCost(char joint, int length, AlgIn *railInfo)
{
	if (length == 0)
		return 0;
...
//return MINIMUM((railInfo -> partsCollection[i].price) +
//			   minimumCost(railInfo -> partsCollection[i].start, length - railInfo -> partsCollection[i].length, railInfo), );
