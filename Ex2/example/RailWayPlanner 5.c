...
railPart *addPartDetail(char **details, int line)
{
	int numParts = line - 3;
	...
	sscanf(details[0], "%c", &partsCollection[numParts].start);
	sscanf(details[1], "%c", &partsCollection[numParts].end);

	char *lenPtr, *pricePtr;
	partsCollection[numParts].length = (int) strtol(details[2], &lenPtr, 10);
	partsCollection[numParts].price = (int) strtol(details[3], &pricePtr, 10);
...
//			printf("LINE (%d) -> %s", line, input);
		...
		else
		{
			...
			printf("LINE (%d) -> ", line);
			printf("[%s][%s][%s][%s]    ", partDetails[0], partDetails[1], partDetails[2], partDetails[3]);

			partsCollection = addPartDetail(partDetails, line);
