...
int addPartDetail(char **details, AlgIn railInfo, const int *capacity, int line)
{
	...
	sscanf(details[0], "%c", &railInfo.partsCollection[numParts].start);
	sscanf(details[1], "%c", &railInfo.partsCollection[numParts].end);

	char *lenPtr, *pricePtr;
	railInfo.partsCollection[numParts].length = (int) strtol(details[2], &lenPtr, 10);
	railInfo.partsCollection[numParts].price = (int) strtol(details[3], &pricePtr, 10);
...
//int minCost(int length, char joint, AlgIn railInfo)
//{
...
int tableBuilder(int rows, int columns, railPart *parts, char joint)
{
	int **table = (int **)malloc(rows * sizeof(int));
	for (int r = 0; r < rows; r++)
	{
		table[r] = (int *)malloc(columns * sizeof(int));
	}
	int *optimalPath = (int *)malloc(columns * sizeof(int));
//	int *table = (int *)malloc(rows * columns * sizeof(int));
	for (int c = 0; c < columns; c++)
	{
		for (int r = 0; r < rows; r++)
		{
			...
			else if (table[r][c-1] == INT_MAX)
			{
				table[r][c] = INT_MAX;
			}
			else if (table[r][c-1] == 0)
			{
				if ((c >= parts[r].length) && (joint == parts[r].end))
				{
					table[r][c] = parts[r].price;
				}
				else
				{
					table[r][c] = 0;
