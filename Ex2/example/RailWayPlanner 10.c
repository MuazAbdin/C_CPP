...
railPart *minCostJoint(char joint, int length, AlgIn *railInfo)
{
//	printf("[ %c ]  [ %D ]", joint, length);
	int minCost = INF;
	railPart *optimalJoint = (railPart *)malloc(sizeof(railPart));
	for (int i = 0; i < railInfo -> numParts; i++)
	{
//		printf("%d", i);
//		printf("%d", railInfo -> partsCollection[i].length);
		if (railInfo -> partsCollection[i].end == joint &&
			railInfo -> partsCollection[i].length <= length)
		{
			if (railInfo -> partsCollection[i].price < minCost)
			{
				optimalJoint = &railInfo -> partsCollection[i];
				minCost = railInfo -> partsCollection[i].price;
			}
		}
	}
//	printf("%d  ", minCost);
//	printf("  %d", railInfo -> partsCollection[0].price);
	return optimalJoint;
}
...
void tableBuilder(int rows, int columns, AlgIn railInfo)
{
	...
	for (int r = 0; r < rows; r++)
	{
		for (int c = 0; c < columns; c++)
		{
			...
			else if (table[(r - 1) * columns + c] == 0 || table[(r - 1) * columns + c] == INF)
			{
				currentPart = minCostJoint(railInfo.kindsJoints[c], r, &railInfo);
//				printf("%c  %c", currentPart->start, currentPart->end);
//				printf("  %d  ", currentPart->price);
//				printf("\n");
				if (currentPart == NULL)
				{
					table[r * columns + c] = INF;
				}
				else
				{
					table[r * columns + c] = currentPart -> price;
				}
			}
			else
			{
				currentPart = minCostJoint(currentPart -> start, r - (currentPart -> length), &railInfo);
				table[r * columns + c] = table[(r - 1) * columns + c] + currentPart -> price;
			}
		}
	}
...
//int jointMinCost(char joint, int length, AlgIn railInfo)
//{
//	int capacity = 1;
...
//railPart *optimalPath(char joint, railPart *sameEnd, AlgIn *railInfo)
//{
//
//
//	int minCost = INF;
//	railPart *optimalJoint = NULL;
//	for (int i = 0; i < railInfo -> numParts; i++)
//	{
//		if (railInfo -> partsCollection[i].end == joint &&
//			railInfo -> partsCollection[i].length <= length)
